...
def abbre_enzyme_list(filename):
    abbre_path = 'Auto-CORPus/output-GWAS/Abbreviations/' + \
        filename.replace('bioc', 'abbreviations')
...
if __name__ == "__main__":
    ...
    output_folder = "Auto-CORPus/Annotated_Corpus/GWAS-Abbre/"
        ...
        for text in fulltext['documents'][0]['passages']:
            ...
            for sentence in paragraph:
                ...
                for word in words:
                    ...
                    if matchword != '':
                        (entity, content, ider) = search_pattern(
                            sentence, word, matchword)
                        if entity != None:
                            w.write(filename + ' ' + section +
                                    ':' + entity + ' ' + ider + '\n')
                            #id_num += 1
                            annotation.append(annotate_corpus(
                                ider, entity, offset, text['text'], section))
                            sentence = content
            if abbre_list:
                end_letter = ['.', ';', '?', '!', '"', ':', ',']
                for word in text['text'].split():
                    ...
                    if word in abbre_list:
                        annotation.append(annotate_abbre(
                            word, text['text'], offset))

            fulltext['documents'][0]['passages'][cont]['annotations'] = annotation
            cont += 1

        enzyme_num += id_num
        writein_path = output_folder + \
            filename.rstrip('.json') + '_' + 'annotated.json'
    ...
    print('The total time of running {} files is: '.format(
        file_num) + str(time.time() - start_time) + '.')
    print('There are {} files which can be found abbreviations of enzymes.'.format(
        abbre_file))
